   ...
   #Make everything into MIPS
    ...
    # OUTPUT 2: Combined channel tifs
            if "C" in mip.dims: # If there are multiple channels in the file
                arr_yxc = np.asarray(mip.transpose("C", "Y", "X")) #create array, grayscale Tif OME-TIFF. 
                out_ome = omero_dir / f"{nd2_file.stem}.ome.tif" # Put these outputs in the omero folder
                if overwrite or not out_ome.exists():
                    imwrite(out_ome, arr_yxc, metadata={"axes": "CYX"}, photometric="minisblack") # preserves metadata for omero/image readers apparently
            else:
...
# Testing function
nd2_to_MIPtif(r("X:/Brain Metastasis Project_AA/Katie Reid - Masters/JI Imaging/se004 NZM7 stacks", r"Y:/Cynthia/se004_NZM7_CGtest", overwrite=TRUE))